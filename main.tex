...
\begin{figure}[htbp]
  ...
  \caption{Various recurrent structures compared with MLP (leftmost).
...
Bengio \cite{bengio1994learning} shows that, though theoretically possible, in practice it's very hard to tune vanilla RNNs to handle long-term dependencies.
...
\begin{figure}[htbp]
  ...
  \caption{Details of an LSTM cell\protect\footnotemark.} \label{fig:lstm-intro}
\end{figure}
...
Results are shown in Figure~\ref{fig:white-noise}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{error_rates_white_noise_added.pdf}
  \caption{Error rates when white noise is added.
...
Results are shown in Figure~\ref{fig:random-noise-same-algorithm} and Figure~\ref{fig:random-noise-same-audio}\footnote{Legends indicate \texttt{(noise, algorithm)} combinations.
...
\begin{figure}[htbp]
  ...
  \caption{Error rates of the same algorithm when random noise is sampled from different audios.
...
\begin{figure}[htbp]
  ...
  \caption{Error rates of different algorithms when random noise is sampled from the same audio.
    Subfigures have different y-axis limits.}
...
\begin{figure}[htbp]
  ...
  \caption{Spectrogram, plot of estimated F0 values and waveform shown in Wavesurfer.
...
And we further distinguish between error types (Figure~\ref{fig:snr-error-types}), which helps us analyze the drawbacks of the algorithms in certain circumstances.
...
\begin{figure}[htbp]
  ...
  \caption{Error rates of the same algorithm when random noise at certain SNR is sampled from different audios.
    Subfigures have different y-axis limits.}
...
\begin{figure}[htbp]
  ...
  \caption{Error rates of different algorithms when random noise at certain SNR is sampled from the same audio.
    Subfigures have different y-axis limits.}
...
\begin{figure}[htbp]
  ...
  \caption{A very small part of the \texttt{pandas} DataFrame storing our data for Martin's algorithm.
